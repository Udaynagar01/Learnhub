...
app.use(
  cors({
    origin(origin, callback) {
      ...
      if (config.nodeEnv === 'development' && localhost.test(origin)) {
...
try {
  await connectDB();
  await ensureDefaultSitePages(SitePage);
  const admin = await ensureAdminAccount();
  console.log(`Admin login: ${admin.email} / ${config.adminPassword}`);
...
server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    ...
    console.error('Stop the old API process, then run npm run dev again.');
    console.error(`Windows: npx kill-port ${config.port}\n`);
  } else {
    console.error(err);
  }
  process.exit(1);
...
server.listen(config.port, () => {
  console.log(`LearnHub API http://localhost:${config.port}`);
});
